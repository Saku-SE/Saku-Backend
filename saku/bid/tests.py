...
class BidUserDeleteTest(TestCase):
    def setUp(self):
        # create user
        self.user = User.objects.create_user(username="test_user", password="Ab654321")
        self.user.is_active = True
    ...
    def test_delete_user(self):
        bid_test = Bid.objects.create(
            user=self.user2,
            auction=self.auction,
            time="2022-08-24T14:15:30Z",
            price=3000,
        )
        
        User.objects.filter(id = self.user2.id).delete()
        self.assertEqual(Bid.objects.filter(id=bid_test.id).first().user.username, "unkown_reserved_user")
